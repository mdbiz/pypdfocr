...
class PyGs(object):
    ...
    def __init__(self, config):
        self.msgs = {
            'GS_FAILED':
            'Ghostscript execution failed',
            'GS_MISSING_PDF':
            'Cannot find specified pdf file',
            'GS_OUTDATED':
            'Your Ghostscript version is probably out of date.  Please upgrade to the latest version',
            'GS_MISSING_BINARY':
            'Could not find Ghostscript in the usual place; please specify it using your config file',
        }
    ...
    def _get_dpi(self, pdf_filename):
        ...
        try:
            ...
            self._warn(
                "Could not execute pdfimages to calculate DPI (try installing xpdf or poppler?), so defaulting to %sdpi"
                % self.output_dpi)
            return
        ...
        if (results[2] != 'image'):
            ...
            return
        x_pt, y_pt, greyscale = int(results[3]), int(
            results[4]), results[5] == 'gray'
        self.greyscale = greyscale

        # Now, run imagemagick identify to get pdf width/height/density
        cmd = 'magick identify -format "%%w %%x %%h %%y" "%s"' % pdf_filename
        try:
            out = subprocess.check_output(cmd, shell=True)
            results = out.splitlines()[0]
            results = results.replace("Undefined", "")
            width, xdensity, height, ydensity = [
                float(x) for x in results.split()
            ]
            ...
            if abs(xdpi - ydpi
                   ) > xdpi * .05:  # Make sure the two dpi's are within 5%
            ...
            logging.debug(str(e))
            self._warn(
                "Could not execute identify to calculate DPI (try installing imagemagick?), so defaulting to %sdpi"
                % self.output_dpi)
        return
    ...
    def make_img_from_pdf(self, pdf_filename):
        ...
        options = ' '.join(self.gs_options[self.img_format][1]) % {
            'dpi': self.output_dpi
        }
