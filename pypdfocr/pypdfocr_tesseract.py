...
class PyTesseract(object):
    ...
    def __init__(self, config):
        ...
        if "binary" in config:  # Override location of binary
            ...
            logging.info("Setting location for tesseracdt executable to %s" %
                         (binary))
        else:
            if str(os.name) == 'nt':
        ...
        self.msgs = {
            ...
            """ 
                ...
                """ % self.binary,
            'TS_VERSION':
            'Tesseract version is too old',
            'TS_img_MISSING':
            'Cannot find specified tiff file',
            'TS_FAILED':
    ...
    def _is_version_uptodate(self):
        ...
        for line in ret_output.splitlines():
            if 'tesseract' in line:
                ...
                if ver_str.endswith(
                        'dev'):  # Fix for version strings that end in 'dev'
                    ver_str = ver_str[:-3]
                if ver_str.startswith(
                        'v'
                ):  # Fix for version strings that add 'v' before version numbers
                    ver_str = ver_str[1:]
    ...
    def make_hocr_from_pnms(self, fns):
        ...
        if not uptodate:
            error(self.msgs['TS_VERSION'] + " (found %s, required %s)" %
                  (ver, self.required))
    ...
    def make_hocr_from_pnm(self, img_filename):
        ...
        logging.info("Running OCR on %s to create %s.html" % (img_filename,
                                                              basename))
