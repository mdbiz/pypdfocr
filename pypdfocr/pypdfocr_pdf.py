...
class PyPdf(object):
    ...
    def mergeRotateAroundPointPage(self, page, page2, rotation, tx, ty):
        ...
        return page.mergeTransformedPage(
            page2,
    ...
    def overlay_hocr_pages(self, dpi, hocr_filenames, orig_pdf_filename):
        ...
        logging.debug(
            "Going to overlay following files onto %s" % orig_pdf_filename)
        ...
        for img_filename, hocr_filename in hocr_filenames:
            text_pdf_filename = self.overlay_hocr_page(dpi, hocr_filename,
                                                       img_filename)
            logging.info(
                "Created temp OCR'ed pdf containing only the text as %s" %
                (text_pdf_filename))
        ...
        for orig_pg, text_pg in zip(
                self.iter_pdf_page(orig), self.iter_pdf_page(text_file)):
    ...
    def _get_merged_single_page(self, original_page, ocr_text_page):
        ...
        if orig_rotation_angle != 0:
            ...
            self.mergeRotateAroundPointPage(
                original_page, ocr_text_page, orig_rotation_angle,
                ocr_text_page.mediaBox.getWidth() / 2,
                ocr_text_page.mediaBox.getWidth() / 2)
    ...
    def overlay_hocr_page(self, dpi, hocr_filename, img_filename):
        ...
        logging.debug("hocr_filename:%s, hocr_dir:%s, hocr_basename:%s" %
                      (hocr_filename, hocr_dir, hocr_basename))
        ...
        with open(pdf_filename, "wb") as f:
            logging.info(
                "Overlaying hocr and creating text pdf %s" % pdf_filename)
      ...
      """
        ...
        for line in page.findall(".//{http://www.w3.org/1999/xhtml}span"):
            ...
            if line.attrib['class'] != 'ocr_line':
                continue
            linebox = self.regex_bbox.search(
                line.attrib['title']).group(1).split()
            ...
            try:
                baseline = self.regex_baseline.search(
                    line.attrib['title']).group(1).split()
            ...
            for word in line:
                ...
                if word.text is None:
                    continue
                logging.debug("word: %s, angle: %d" % (word.text.strip(),
                                                       textangle))

                box = self.regex_bbox.search(
                    word.attrib['title']).group(1).split()
                ...
                normal.leading = 0
                font_name, font_size = self._get_font_spec(
                    word.attrib['title'])
                normal.fontName = "Helvetica"
                normal.fontSize = font_size

                para = RotatedPara(
                    escape(word.text.strip()), normal, textangle)
                para.wrapOn(pdf, para.minWidth(),
                            100)  # Not sure what to use as the height  here
