...
class TestPydfocr:
    ...
    pdf_tests = [
        (".", os.path.join("temp", "target", "recipe"),
         os.path.join("..", "test", "pdfs", "test_recipe.pdf"), [
             ["Simply Recipes"],
         ]),
        (
            ".",
    ...
    def test_standalone_email(self, dirname, tgt_folder, filename, expected):
        ...
        with patch("smtplib.SMTP") as mock_smtp:
            ...
            os.chdir(dirname)
            opts = [
                filename, "--preprocess", "--config=test_pypdfocr_config.yaml",
                "-m"
            ]
            self.p.go(opts)
    ...
    @pytest.mark.parametrize("dirname, tgt_folder, filename, expected",
                             pdf_tests[0:3])
    def test_standalone_filing(self, mock_move, config, dirname, tgt_folder,
                               filename, expected):
        """
        ...
        calls = [
            call(out_filename,
                 os.path.abspath(
                     os.path.join(tgt_folder, os.path.basename(out_filename))))
        ]
        if not "no_move_original" in config:
            new_file_name = os.path.basename(filename).replace(
                ".pdf", "_2.pdf")
            calls.append(
                call(filename,
                     os.path.abspath(
                         os.path.join("temp", "original", new_file_name))))
