...
def build_windows_dist():
    if os.name == 'nt':
        ...
        local(
            "python ../pyinstaller/pyinstaller.py pypdfocr_windows.spec --onefile"
        )
...
def run_tests():
    ...
    with lcd(test_dir):
        ...
        t = local(
            "py.test --cov-config .coveragerc --cov=pypdfocr --cov-report=term --cov-report=html",
            capture=False)
        t = local("coveralls")
...
def push_docs():
    ...
    with lcd("docs"):
        print(
            "Running sphinx in docs/ and building to ~/dev/githubpages/pypdfocr"
        )
        local("make clean")
        local("make html")
